...
export const handler = async (awsEvent: AWSEvent) => {
  ...
  if (!process.env.EVENTS_TABLE_NAME) throw new Error('Missing process.env.EVENTS_TABLE_NAME');
  ...
  // @ts-ignore
  const events = await Promise.all(items.sort((a, b) => a.dynamodb?.NewImage?.timestamp?.N - b.dynamodb?.NewImage?.timestamp?.N).map(async (record) => {
    if (record.eventName !== 'INSERT') return undefined;

    if (!record.dynamodb?.Keys) return undefined;

    // TODO: deserialize data from DDB stream 
    const res = await ddbClient.query({
