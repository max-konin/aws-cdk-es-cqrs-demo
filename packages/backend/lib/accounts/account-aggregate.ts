import { CREDIT_ACCOUNT_COMMAND_TYPE, DEBIT_ACCOUNT_COMMAND_TYPE, OPEN_ACCOUNT_COMMAND_TYPE } from '../commands';
import { ACCOUNT_CREDITED_EVENT_TYPE, ACCOUNT_DEBITED_EVENT_TYPE, ACCOUNT_OPENED_EVENT_TYPE } from '../events';
...
// OPEN ACCOUNT
const executeOpenAccountCommand = (state: AccountAggregateState, cmd: OpenAccountCommand) => {
  if (state.accountId) return { ok: false, error: 'Account already exists' }

  return { ok: true, events: [new AccountOpenedEvent(cmd.data)]}
}

const applyAccountOpenedEvent = (state: AccountAggregateState, cmd: OpenAccountCommand) => {
  return {
...
const executeCreditAccountCommand = (state: AccountAggregateState, cmd: CreditAccountCommand) => {
  if (!state.accountId) return { ok: false, error: 'not found'};

  return { ok: true, events: [new AccountCreditedEvent(cmd.data)]}
}

const applyAccountCreditedEvent = (state: AccountAggregateState, event: AccountCreditedEvent) => ({
  ...state,
...
const executeDebitAccountCommand = (state: AccountAggregateState, cmd: DebitAccountCommand) => {
  if (!state.accountId) return { ok: false, error: 'not found'}
  if (state.balance < cmd.data.amount) return { ok: false, error: 'not enough money'};

  return { ok: true, events: [new AccountDebitedEvent(cmd.data)]}
}

const applyAccountDebitedEvent = (state: AccountAggregateState, event: AccountDebitedEvent) => ({
  ...state,
  balance: state.balance - event.data.amount,
})
